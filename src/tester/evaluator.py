...
class UnifiedDatasetEvaluator(DatasetEvaluator):
    ...
    def process_wic(self, inputs, outputs, dataset_name: str):
        for input, output in zip(inputs, outputs):
            ...
            self._predictions.append(
                dict(image_contains_mixed_unknowns=torch.tensor([is_unknown] * len(correct)).to(correct.device),
    ...
    def evaluate(self):
        ...
        if self.mode == "open_cwwr":
            ...
            wi_adjusted = self.compute_WI_adjusted_at_many_recall_level(all_recs, tp_css_cum, fp_css_cum, tp_oss_cum,
                                                                        fp_oss_cum)
            wi_no_simplified = self.compute_WI_no_simplified_at_many_recall_level(all_recs, tp_css_cum, fp_css_cum,
                                                                                  tp_oss_cum, fp_oss_cum)
    ...
    def evaluate_wic(self):
        ...
        del self.all_predictions
        image_contains_mixed_unknowns = [el for prediction in predictions for el in
    ...
    def wic_analysis(self, eval_info, Recalls_to_process, wilderness):
        ...
        eval_predictions["image_contains_mixed_unknowns"] = torch.stack(
            eval_predictions["image_contains_mixed_unknowns"])
    ...
    def compute_WI_no_simplified_at_a_recall_level(self, recalls, tp_cs, fp_cs, tp_os, fp_os, recall_level=0.5):
        wi_no_simplified = []
        index_tp_os = min(range(len(recalls[-1])),
                          key=lambda i: abs(recalls[-1][i] - recall_level)) if self.model_reject else -1
        for cls_id, rec in enumerate(recalls[:-1]):
            index = min(range(len(rec)), key=lambda i: abs(rec[i] - recall_level))
            tp_cs_cls = tp_cs[cls_id][index]
            fp_cs_cls = fp_cs[cls_id][index]
            ...
            wi_no_simplified_cls = tp_cs_cls / (tp_cs_cls + fp_cs_cls) * (
                        tp_cs_cls + tp_os_cls + fp_os_cls + fp_cs_cls) / (tp_cs_cls + tp_os_cls) - 1
