...
class MyRPN(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret["post_training"] = cfg.OURS.POST_TRAINING
        ret["train_rcnn"] = cfg.OURS.TRAIN_RCNN
        return ret
    ...
    @torch.no_grad()
    def select_fg_bg_proposals_score(self, predictions_logits, labels):
        fg_proposals_scores = []
        bg_proposals_scores = []
        for predictions_logits_i, labels_i in zip(predictions_logits, labels):
            fg_scores = predictions_logits_i[0][labels_i[0] == 1]
            bg_scores = predictions_logits_i[0][labels_i[0] == 0]
            fg_proposals_scores.append(fg_scores)
            bg_proposals_scores.append(bg_scores)

        return fg_proposals_scores, bg_proposals_scores
    ...
    def forward(
        ...
        if self.post_training:
            ...
            fg_scores, bg_scores = self.select_fg_bg_proposals_score(pred_objectness_logits, gt_labels)
            return fg_scores, bg_scores
        elif self.training:
