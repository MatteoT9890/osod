...
__all__ = ["load_all_instances", "load_subset_instances", "voc_register", "DataSplits", "coco_register"]
logger = logging.getLogger(__name__)


class DataSplits:
    ALL: str = "all_class"
    KNOWN: str = "known_class"
    UNKNOWN: str = "unknown_class"
...
def load_known_and_unknown_instances(dirname: str, split: str, known_class_names: Union[List[str], Tuple[str, ...]], debug=False):
    ...
    for fileid in fileids:
        ...
        for obj in tree.findall("object"):
            ...
            if cls in known_class_names:
                category_id = known_class_names.index(cls)
            else:
                category_id = known_class_names.index("unknown")
...
def load_unknown_instances(dirname: str, split: str, known_class_names: Union[List[str], Tuple[str, ...]]):
    """
    Load object detection annotations to Detectron2 format.
...
def voc_register(dirname, split, year, data_splits, debug=False):
    ...
    DatasetCatalog.register(known_and_unknown_class_split.dataset_name,
    ...
    MetadataCatalog.get(known_and_unknown_class_split.dataset_name).set(
        thing_classes=list(known_and_unknown_class_split.class_names), dirname=dirname, year=year, split=split,
        class_ids=list(range(len(list(known_and_unknown_class_split.class_names))))
    )
...
def register_coco_instances(name, json_file, image_root, unk_id, debug=False):
    ...
    MetadataCatalog.get(name).set(
        ...
        thing_classes=["unknown"], class_ids=[unk_id], map_unk_id={0: unk_id}
    )
